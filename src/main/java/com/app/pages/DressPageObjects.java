...
public class DressPageObjects extends BasePage{
    ...
    /* Verify Product name sort from A to Z */
	
	public void SelectAtoZ(String optionTobeSelected){
		bp.selectFromDropDown(Sortbydropdown, optionTobeSelected);
		bp.isElementVisible(Loadicon);
		bp.isElementinVisible(Loadicon);
	}
	...
	public List<String> getProductname(){
		...
		for(WebElement element : lstproductNames){
			String AtoZname = element.getText();
			System.out.println("Product name:" + AtoZname);
			list.add(AtoZname);
		}
		return list;
	}
  ...
  /** Verify the reduced price of dresses using search icon **/
    ...
    private WebElement priceReduced;
    
    @FindBy(xpath = "//div[@class='right-block-content row']//span[@class='price-percent-reduction']")
    private WebElement discountPercent;
    
    
    public void checkDiscount(){
    	Searchicon.sendKeys("Printed");
    	Submitsearch.click();
    	bp.Explicitwait(listIcon);
    	listIcon.click();
    	
    	if(bp.Displaycheck(priceReduced)){
    		bp.Displaycheck(discountPercent);
    	}else { 
    	System.out.println("Discount amount not displayed");
    	}
